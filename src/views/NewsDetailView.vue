...
<div else>
    ...
    <div class="container mx-auto space-y-5 my-5 py-5 rounded-lg dark:bg-gray-900 shadow-md border dark:border-blue-500 dark:text-gray-300" v-for="(item) in filteredItems" :key="item.id">
...
export default {
    ...
    computed: {
        filteredItems() {
            return this.newsData.filter(item => item.language === this.$i18n.locale)
        }
    },
    methods: {
        formatedDate(item) {
    ...
    mounted() {
        ...
        axios.get(`/api/Depo/GetDepoNewsById${id}`)
            .then(response => {
