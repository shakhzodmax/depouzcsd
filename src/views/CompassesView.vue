<template>
    ...
    <div class="mb-10 2xl:mx-0 xl:mx-0 lg:mx-0 mx-2">
      <div
          ...
          <CompassBlock @click="storeCompass(item.id)" v-for="item in this.deposDate" :key="item.id" :title="
...
export default {
  ...
  methods: {
    ...
    clearSearch() {
      ...
      this.axios.get(`/api/Depo/GetLastPageNumberDepoCompass/{pageSize}/${this.itemsPerPage}`)
          .then(response => {
    ...
    goToPage(pageNumber) {
      this.loading = true;
      this.axios.get(`/api/Depo/GetDepoCompassByPageNumber/${pageNumber}/${this.itemsPerPage}`)
          .then(response => {
            this.deposDate = response.data;
          })
          .catch(error => {
