...
<div v-else>
    <div class="bg-gradient-to-tr from-emerald-600 via-emerald-500 to-sky-400 h-56 w-full flex items-center dark:bg-gradient-to-tr dark:from-gray-900 dark:via-slate-800 dark:to-gray-900">
        <div class="container mx-auto">
            <div class="2xl:border-l-4 xl:border-l-4 lg:border-l-4 md:border-l-4 border-white dark:border-emerald-600 2xl:text-start xl:text-start md:text-start text-center">
    ...
    <section>
        <div class="container mx-auto py-5">
            <div class="grid 2xl:grid-cols-2 xl:grid-cols-2 lg:grid-cols-2 md:grid-cols-2 grid-cols-1 p-5 mx-2 dark:border border-emerald-500 rounded-lg dark:bg-gray-900 space-x-4 border shadow" v-for="(item) in filteredItems" :key="item.id">
                ...
                <div class="basis-3/5 flex flex-col justify-center">
                    <div class="grid 2xl:grid-cols-2 grid-cols-1">
                        <p class="font-bold text-emerald-500 dark:text-emerald-500 2xl:text-start text-center 2xl:text-2xl xl:text-2xl lg:text-2xl text-xl mb-2">{{ item.title }}</p>
                        <div class="flex items-center 2xl:justify-end justify-center space-x-2 mb-2">
                    ...
                    <a target="_blank" :href="'/api/Website/GetPdfById'+  item.pdfId" type="button" class="my-5 text-white bg-gradient-to-br from-emerald-500 to-emerald-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-emerald-200 dark:focus:ring-emerald-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center">{{ $t('others.watch') }}</a>
                </div>
            </div>
        </div>
    </section>
...
export default {
    ...
    mounted() {
        let id = parseInt(sessionStorage.getItem('MESSAGE_ID'));
