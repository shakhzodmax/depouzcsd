<template>
  <div class="bg-gradient-to-tr from-indigo-600 via-indigo-500 to-sky-500 h-56 w-full flex items-center dark:bg-gradient-to-tr dark:from-gray-900 dark:via-slate-800 dark:to-gray-900">
      <div class="container mx-auto">
          <div class="2xl:border-l-4 xl:border-l-4 lg:border-l-4 md:border-l-4 border-white dark:border-blue-600 2xl:text-start xl:text-start md:text-start text-center">
              <h1 class="font-bold text-4xl text-white mb-2 indent-2 dark:text-blue-500">{{ $t('main.compass') }}</h1>
          </div>
  ...
  <!-- <div class="container mx-auto space-y-5 my-5 py-5 rounded-lg dark:bg-gray-900 shadow-md border dark:border-lime-500 dark:text-gray-300" v-for="(item) in filteredItems" :key="item.id">
      <div class="text-center">
          <p class="text-2xl font-bold text-orange-500 dark:text-blue-500">{{ item.title }}</p>
      </div>
      ...
      <div class="flex justify-center text-center space-x-2 text-orange-500">
...
<section>
    <div class="container mx-auto">
    <div class="flex justify-center items-center" v-for="(item) in filteredItems" :key="item.id">
        <img :src="item.coverBase64" alt="" class="mx-auto basis-2/5 scale-75 shadow-md" />
        <div class="basis-3/5 flex flex-col justify-between space-y-5">
            <p class="text-3xl font-bold text-indigo-500 dark:text-blue-500 text-center">{{ item.title }}</p>
            <div v-html="item.description" class="indent-10 text-justify dark:text-gray-400"></div>
            <a :href="'/api/Website/GetPdfById'+  item.pdfId" type="button" class="text-white bg-gradient-to-br from-indigo-500 to-sky-400 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-indigo-200 dark:focus:ring-indigo-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center">{{ $t('others.watch') }}</a>
        </div>
    </div>
    </div>
</section>
